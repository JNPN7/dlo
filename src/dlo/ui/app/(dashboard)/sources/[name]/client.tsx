...
export function SourceDetailClient({ params }: SourceDetailClientProps) {
  const { name } = use(params);
  const decodedName = decodeURIComponent(name);
  const { getSource, isLoading, error } = useManifest();

  const source = getSource(decodedName);
          ...
          <Card>
            <CardContent className="pt-6">
              <p className="text-muted-foreground">
                {error?.message || `Source "${decodedName}" not found`}
              </p>
              <Link href="/sources">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <CardDescription className="flex items-center gap-2 mt-1">
                  ...
                  {source.details?.type && <Badge variant="outline">{source.details.type}</Badge>}
                </CardDescription>
              </div>
            </div>
          </CardHeader>
