...
export function ModelDetailClient({ params }: ModelDetailClientProps) {
  const { name } = use(params);
  const decodedName = decodeURIComponent(name);
  const { getModel, isLoading, error } = useManifest();

  const model = getModel(decodedName);
          ...
          <Card>
            <CardContent className="pt-6">
              <p className="text-muted-foreground">
                {error?.message || `Model "${decodedName}" not found`}
              </p>
              <Link href="/models">
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center gap-3">
              <div
                className={`flex h-10 w-10 items-center justify-center rounded-lg ${typeColors[model.type] || typeColors.materialized}`}
              >
              ...
              <div>
                ...
                <CardDescription className="flex items-center gap-2 mt-1">
                  ...
                  {model.compiled && <Badge variant="outline">compiled</Badge>}
                </CardDescription>
              </div>
            </div>
          </CardHeader>
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              ...
              {model.schedule && (
                <div className="space-y-1">
                  ...
                  <code className="text-xs bg-muted px-2 py-1 rounded block">{model.schedule}</code>
                </div>
              )}
