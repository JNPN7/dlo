...
export function ColumnTable({ columns }: ColumnTableProps) {
  if (columns.length === 0) {
    return <div className="text-center py-8 text-muted-foreground">No columns defined</div>;
  }
      ...
      <TableBody>
        {columns.map((column) => (
          <TableRow key={column.name}>
            ...
            <TableCell>
              <code className="text-xs bg-muted px-1.5 py-0.5 rounded">{column.type}</code>
            </TableCell>
            <TableCell>
              {column.category && (
