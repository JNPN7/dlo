...
import { GitBranch, ArrowRight, Tag, ExternalLink } from 'lucide-react';
...
export function RelationshipCard({ relationship }: RelationshipCardProps) {
  return (
    <Link to={`/relationships/${relationship.name}`}>
      <Card className="h-full transition-all hover:shadow-lg hover:border-primary/50 group">
          ...
          <div className="flex items-start justify-between">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <Badge variant="relationship" className="text-xs mt-1">
                  relationship
                </Badge>
              </div>
            </div>
        ...
        <CardContent className="space-y-3">
          ...
          <div className="text-xs text-muted-foreground">
            ...
            {relationship.from_columns.join(', ')} = {relationship.to_columns.join(', ')}
          </div>
